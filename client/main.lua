...
AddEventHandler('esx_communityservice:inCommunityService', function(actions_remaining)
	...
	isSentenced = false
	TriggerServerEvent("fx-clothes:loadPlayerSkin") --CHANGE HERE
end)
		...
		local DataFemale = {
				...
				["pants"]  ={item =36, texture = 0},
				 ...
				 if Player.charinfo.gender == 0 then
					 ...
					 TriggerEvent('fx-clothing:client:loadOutfit', DataMale) --Change Here the Clothing resource

								 else

				 	TriggerEvent('fx-clothing:client:loadOutfit', DataFemale) --CHANGE HERE TOO
